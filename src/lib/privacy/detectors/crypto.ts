...
const keys =
  /\b(?:sk-[A-Za-z0-9_-]{20,}|ghp_[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}|[0-9a-fA-F]{64})\b/g;
const words = new Set(
...
export const detectSecrets: Detector = (text) => {
  const found = [...text.matchAll(keys)].map((m) => ({
  ...
  for (const m of text.matchAll(/\b[A-Za-z0-9_-]{32,}\b/g)) {
    ...
    if (!value.startsWith("0x") && classes >= 3 && diversity > 0.45)
      found.push({
        type: "SECRET",
        start: m.index ?? 0,
  ...
  for (const m of text.matchAll(
    /\b(?:[a-z]{3,}\s+){11}[a-z]{3,}\b|\b(?:[a-z]{3,}\s+){23}[a-z]{3,}\b/gi,
  )) {
    const tokens = m[0].toLowerCase().split(/\s+/);
    if (tokens.every((word) => words.has(word)))
      found.push({
        type: "SECRET",
        start: m.index ?? 0,
        end: (m.index ?? 0) + m[0].length,
